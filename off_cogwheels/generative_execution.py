...
async def execute_async_code(code, inter, file):
    try:
        exec(
            f"async def __async_exec_func__(inter, file):\n"
            + "\n".join(f"    {line}" for line in code.splitlines()),
            globals()
        )
        await eval("__async_exec_func__(inter, file)", globals(), {"inter": inter, "file": file})
        await inter.followup.send("Запрос выполнен! Выполненный код прикреплён.", file=file)
...
class WishCog(commands.Cog):
    ...
    @commands.slash_command(name="grant-a-wish", description="исполняет любое дискорд желание", guild_ids=GUILD_IDS)
    async def command(self,
        ...
        model = genai.GenerativeModel(
            ...
            system_instruction=[
                ...
                f"{open("cogwheels/generative_execution.py", "r", encoding="utf-8").read()}\n\n"
        ...
        try:
            ...
            return
        if code.lower() == "no":
            await inter.followup.send(f"Запрос посчитан невыполнимым.")
            return

        file_obj = StringIO(code)
        file = disnake.File(fp=file_obj, filename="code.txt")

        await execute_async_code(code, inter, file)  # YOUR CODE WILL BE EXECUTED HERE
