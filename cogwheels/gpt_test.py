...
class TestCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.active_channel_ids: dict[int: Conversation] = dict()

    @commands.slash_command(name="generate-text", description="generates text", guild_ids=GUILD_IDS)
    async def generate_text(self, inter: disnake.ApplicationCommandInteraction, prompt: str):
    ...
    @commands.slash_command(name="generate-image", description="generates image", guild_ids=GUILD_IDS)
    ...
    @commands.slash_command(name="webhook-test", description="webhook test", guild_ids=GUILD_IDS)
    async def webhook_test(self,
    ...
    @commands.slash_command(name="activate-ai", description="активировать терминатора", guild_ids=GUILD_IDS)
    async def activate_ai(self,
                          inter: disnake.ApplicationCommandInteraction):
    ...
    @commands.slash_command(name="deactivate-ai", description="деактивировать терминатора", guild_ids=GUILD_IDS)
    async def deactivate_ai(self,
                            inter: disnake.ApplicationCommandInteraction):
        if inter.author.id != inter.guild.owner_id:
            await inter.response.send_message("Only the server owner can use this command!", ephemeral=True)
            return
