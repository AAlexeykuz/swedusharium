...
class Conversation:
    ...
    def get_response(self):
        response = self.client.chat.completions.create(
            model="gpt-4o-mini", messages=self.history, web_search=False
        )
...
class TestCog(commands.Cog):
    ...
    async def generate_text(self, inter: disnake.ApplicationCommandInteraction, prompt: str):
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini", messages=[{"role": "user", "content": prompt}], web_search=False
        )
        text = response.choices[0].message.content
        await inter.followup.send(text)

    @commands.slash_command(
        name="generate-image", description="generates image", guild_ids=GUILD_IDS
    )
    ...
    @commands.slash_command(
        name="activate-ai", description="активировать терминатора", guild_ids=GUILD_IDS
    )
    ...
    @commands.slash_command(
        name="deactivate-ai", description="деактивировать терминатора", guild_ids=GUILD_IDS
    )
